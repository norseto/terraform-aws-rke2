resource "aws_ec2_fleet" "this" {
  ...
  on_demand_options {
    allocation_strategy = try(local.fleet_strategy_dict_on_demand[each.value.instances_distribution.on_demand_allocation_strategy], "lowestPrice")
  }

  spot_options {
    instance_interruption_behavior = "stop"
    allocation_strategy            = try(local.fleet_strategy_dict_spot[each.value.instances_distribution.spot_allocation_strategy], "lowestPrice")
  }
